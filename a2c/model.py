...
class ActorCritic(nn.Module):
	def __init__(self, a_dim=18):
		super(ActorCritic, self).__init__()
		self.a_dim = a_dim
		
		# actor model
		self.a_fc0 = nn.Linear(1, 128) # buffer size 1 / 1 
		...
		# critic model
		self.c_fc0 = nn.Linear(1, 128) # buffer size 1 / 1
	...
	def forward(self, inputs, batch_size=1):
		...
		split_5 = F.relu(self.a_fc5(inputs[:, 4:5, -1]))
		split_6 = F.relu(self.a_fc6(inputs[:, 5:6, -1]))
		split_7 = F.relu(self.a_conv7(inputs[:, 6:7, 0:16])).view(batch_size, -1)
		split_8 = F.relu(self.a_conv8(inputs[:, 7:8, 0:16])).view(batch_size, -1)
		...
		split_5 = F.relu(self.c_fc5(inputs[:, 4:5, -1]))
		split_6 = F.relu(self.c_fc6(inputs[:, 5:6, -1]))
		split_7 = F.relu(self.c_conv7(inputs[:, 6:7, 0:16])).view(batch_size, -1)
		split_8 = F.relu(self.c_conv8(inputs[:, 7:8, 0:16])).view(batch_size, -1)
